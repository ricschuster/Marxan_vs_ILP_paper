...
runs <- foreach(run = seq_len(nrow(runs)), .combine = bind_rows, .packages = pkg_list) %dopar% {
  r <- runs[run, ]
  ...
  #                     .combine = bind_rows, .packages = pkg_list) %dopar% {
                        ...
                        # options
                        m_opts <- MarxanOpts(NCORES = 1L, VERBOSITY = 3L)
                        m_opts@BLM <- as.integer(r$blm)
                        ...
                        r_marxan <- select(r_marxan, marxan_iterations, spf, n_solutions, cost, time)
                      # }
  r$marxan <- list(r_marxan)
  # save this iteration in case of crashing
  str_glue_data(r, "Marxan_run-", run,
