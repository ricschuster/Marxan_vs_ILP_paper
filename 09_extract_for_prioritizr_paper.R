...
# tmp.r <- crop(tt, e)
# 
# df_red <- tibble(pu = tmp.r[]) %>%
#   left_join(cost_occ, by = "pu")
# 
# ll <- list()
# for(ii in 1:ncol(df_red)){
#   ll[[ii]] <- tmp.r
#   values(ll[[ii]]) <- df_red %>% pull(ii)
#   names(ll[[ii]]) <- names(df_red)[ii]
# }
# ll2 <- stack(ll)
# 
# # writeRaster(ll2, )
# writeRaster(ll2, filename = here("/tmp", names(ll2)) , bylayer=TRUE,format="GTiff")
# 

#END PREP
ll2 <- list.files(here("tmp"), pattern = ".tif$", full.names = TRUE) %>%
  stack() %>%
  readAll()


con_problem <-
  prioritizr::problem(ll2$cost, round(ll2 %>% dropLayer(c(1,2)), 4)) %>%
  prioritizr::add_min_set_objective() %>%
  prioritizr::add_relative_targets(0.2) %>%
  # prioritizr::add_locked_in_constraints(con_pa_raster_data) %>%
  prioritizr::add_binary_decisions() %>%
  prioritizr::add_gurobi_solver(gap = 0.01,
                                threads = 1,
                                first_feasible = FALSE)


boundary_penalties <- c(0, 100, 1000)

con_boundary_raster_solutions <-
  boundary_penalties %>%
  lapply(function(x) {
    con_problem %>%
      prioritizr::add_boundary_penalties(x, edge_factor = 0.5) %>%
      prioritizr::solve(force = TRUE)
  }) %>% raster::stack()
raster::plot(con_boundary_raster_solutions)
